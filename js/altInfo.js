...
function processAlternatives(result) {
	...
	for (var i in js) {
		...
		if (alternativeDesc != "") {
			...
			if(isLoggedIn != "0"){
				output = output + "<h4 style=\"font-family:verdana\"> Alternative " + count + ": </h4><label>" + alternativeDesc + "</label> " + approvalButtons + disapprovalButtons  + "<div id=\"approvals" + count + "\"></div>" + "<div id=\"feedback" + count + "\"></div>";
			}
			else{
